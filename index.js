...
const MIO_NUMERO = '393887347002'; // NUMERO GIUSTO PER IL PAIRING CODE
const MONETA = 'schei';
...
const BLOCCHI = [
  ...
  { tipo: 'comando', desc: 'I miei canali', nomi: ['social','canali'], async run(ctx){ const {sock,m,from,reply,V}=ctx; await sock.sendMessage(from,{ text: F('📡 *Dove mi trovi*\n\nCanale sicurezza, aggiornamenti del bot e roba nuova. scrivimi in privato.',V) },{quoted:m}); } },
  ...
  { tipo: 'comando', desc: 'Regala monete a chi tagghi', nomi: ['paga','dona'], soloGruppo:true, async run(ctx){ const {reply,V,sender,target}=ctx; const n=parseInt(V.arg1,10); const amount=isNaN(n)?100:n; addSaldo(sender,-amount); addSaldo((target||sender),amount); V.saldo=getSaldo(sender); V.arg1=amount; await reply(F('📤 Hai mandato {arg1} {moneta} a {target}. Totale tuo: {saldo}',V)); } },
  ...
  { tipo: 'comando', desc: 'Cerca e manda un audio', nomi: ['play','musica','song'], async run(ctx){ const {sock,m,from,reply,V}=ctx; const cerca=String(V.args||"").replace(/["`$\\]/g,""); if(!cerca){ await reply("Scrivi cosa cercare, es. "+PREFIX+V.cmd+" imagine dragons believer"); } else { await reply(F('🔎 Cerco *{args}*, un attimo…',V)); const base=TMP+"/au"+Date.now(); try{ await sh("yt-dlp -x --audio-format mp3 --audio-quality 5 --no-playlist --match-filter \"duration < 600\" -o \""+base+".%(ext)s\" \"ytsearch1:"+cerca+"\""); const file=base+".mp3"; if(!fs.existsSync(file)) throw new Error("niente"); await sock.sendMessage(from,{audio:fs.readFileSync(file),mimetype:"audio/mpeg",ptt:false},{quoted:m}); pulisci([file]); }catch(e){ await reply("Audio non trovato o troppo lungo."); } } } },
  ...
  { tipo: 'comando', desc: 'Solo admin modificano le info', nomi: ['blocca','lock'], soloGruppo:true, soloOwner:true, async run(ctx){ const {sock,from,reply,V}=ctx; await sock.groupSettingUpdate(from,'locked'); await reply(F('🔒 Nome, foto e descrizione ora si toccano solo da admin.',V)); } },
  { tipo: 'comando', desc: 'Tutti modificano le info', nomi: ['sblocca','unlock'], soloGruppo:true, soloOwner:true, async run(ctx){ const {sock,from,reply,V}=ctx; await sock.groupSettingUpdate(from,'unlocked'); await reply(F('🔓 Info del gruppo di nuovo libere.',V)); } },
  { tipo: 'comando', desc: 'Tagga tutto il gruppo', nomi: ['tagall','everyone','all'], soloGruppo:true, soloAdmin:true, async run(ctx){ const {sock,from,reply,V}=ctx; const mem=(await sock.groupMetadata(from)).participants.map(x=>x.id); await sock.sendMessage(from,{text:F('📢 *Avviso da {nome}*\n{args}',V)+"\n"+mem.map(at).join(" "),mentions:mem}); } },
  ...
  { tipo: 'comando', desc: 'Rende admin il taggato', nomi: ['promuovi','admin','up'], soloGruppo:true, soloAdmin:true, async run(ctx){ const {sock,from,reply,V,target}=ctx; if(target) await sock.groupParticipantsUpdate(from,[target],'promote'); await reply(F('⬆ {target} ora è admin.',V)); } },
  { tipo: 'comando', desc: 'Toglie admin al taggato', nomi: ['declassa','unadmin','down'], soloGruppo:true, soloAdmin:true, async run(ctx){ const {sock,from,reply,V,target}=ctx; if(target) await sock.groupParticipantsUpdate(from,[target],'demote'); await reply(F('⬇ {target} non è più admin.',V)); } },
  { tipo: 'comando', desc: 'Cancella il messaggio citato', nomi: ['elimina','del','delete'], soloGruppo:true, soloAdmin:true, async run(ctx){ const {sock,from,m}=ctx; await sock.sendMessage(from,{delete:m.key}); } },
  ...
  { tipo: 'benvenuto', desc: 'Saluta i nuovi', async run(ctx){ const {sock,from,m,V}=ctx; await sock.sendMessage(from,{text:F('👋 Benvenuto {tag} in *{gruppo}*!\nSiamo in {membri}.',V)}); } },
  { tipo: 'addio', desc: 'Saluta chi esce', async run(ctx){ const {from,reply,V}=ctx; await reply(F('👋 {tag} ha lasciato il gruppo. Restiamo in {membri}.',V)); } },
...
async function onMessage(sock, m) {
  try {
    ...
    if (isGroup) {
      try { meta = await sock.groupMetadata(from); admins = meta.participants.filter((p) => p.admin).map((p) => p.id); } catch (e) {}
    }
    ...
    for (const b of BLOCCHI) {
      let ok = false, cmd = "";
      if (b.tipo === "comando" && low.startsWith(PREFIX)) {
        const primo = low.slice(PREFIX.length).split(/\s+/)[0];
        if (b.nomi.includes(primo)) { ok = true; cmd = primo; }
      ...
      const V = makeV({
        nome: m.pushName || "utente", user: sender, id: sender, tag: at(sender), chat: from,
        gruppo: meta? meta.subject : "chat privata", descr: meta? meta.desc || "" : "", membri: meta? meta.participants.length : 1,
        admin: isAdmin? "sì" : "no", owner: isOwner? "sì" : "no", isgruppo: isGroup? "sì" : "no",
        saldo: getSaldo(sender), saldotarget: target? getSaldo(target) : 0, cmd: cmd,
        args: body.slice((PREFIX + cmd).length).trim(),
        arg1: body.slice((PREFIX + cmd).length).trim().split(/\s+/)[0] || "",
        arg2: body.slice((PREFIX + cmd).length).trim().split(/\s+/)[1] || "",
        arg3: body.slice((PREFIX + cmd).length).trim().split(/\s+/)[2] || "",
        target: target? at(target) : "nessuno", targetnome: target || "nessuno",
        citato: citato(m), tipo: tipoMsg(m), totcmd: BLOCCHI.filter(x=>x.tipo==="comando").length
      });
      const ctx = { sock, m, from, sender, target, reply, at, V, t0 };
      await b.run(ctx);
      break;
    }
  } catch (e) { console.log("Errore messaggio:", e) }
}
...
async function start() {
  ...
  if (!sock.authState.creds.registered) {
    setTimeout(async () => {
      ...
      } catch (e) { console.log("Errore pairing:", e) }
    }, 3000);
  }
  sock.ev.on('connection.update', (u) => {
    ...
    } else if (connection === 'open') console.log('Connesso! Bot attivo su', MIO_NUMERO);
  });
  ...
  sock.ev.on('group-participants.update', async (upd) => {
    try {
      const meta = await sock.groupMetadata(upd.id);
      for (const p of upd.participants) {
        const mFake = { key: { remoteJid: upd.id }, pushName: p.split("@")[0], message: {} };
        const V = makeV({ tag: at(p), gruppo: meta.subject, membri: meta.participants.length, chat: upd.id });
        const ctx = { sock, m: mFake, from: upd.id, sender: p, target: p, reply: (t)=>sock.sendMessage(upd.id,{text:t}), at, V, t0: Date.now() };
        if (upd.action === 'add') { const b = BLOCCHI.find(x=>x.tipo==='benvenuto'); if(b) await b.run(ctx); }
        if (upd.action === 'remove') { const b = BLOCCHI.find(x=>x.tipo==='addio'); if(b) await b.run(ctx); }
      }
    } catch(e){}
  });
}
